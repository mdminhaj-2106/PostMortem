...
- `ingest.py`: memoizes reconciled days per `(episode_id, kpi_name, day_offset)` (audit finding F13) — one Stage 3 run re-requests the same days five times (three `run_stage2` passes for F3's relationship wiring + two `load_dollar_residuals`), and each day is real Neon round trips. Measured on the episode-8 demo: Stage 3 **111.3s → 49.9s**, whole demo 174.6s → 112.8s. Per *day*, not per timeline, so partially-overlapping windows still hit. Note the cache cannot be shared across the bridge modules — each `sys.modules`-eviction re-import gives Stage 3/4 their own module instance and therefore their own cache, so the win is within a stage, not across the demo; a real root package would recover the rest. `clear_cache()` exists for the one case where frozen data isn't a safe assumption.
