...
pipeline/stage03_cross_kpi_correlation/
  ...
  stage2_bridge.py              — pulls Stage 2 results + Stage 2's baseline residuals per KPI, via sys.path insert (Stage 2's own import pattern, one stage further; named stage2_bridge.py not ingest.py -- Stage 2 already has its own ingest.py, and this module needs to import_module("ingest") to reach it)
