...
- No root-level Python package/manifest yet — each pipeline module is independently venv'd. This has now bitten twice: Stage 2 importing Stage 1's `reconcile.py`, and now Stage 3 importing Stage 2's `stage2`/`ingest`/`baseline` (`stage03_cross_kpi_correlation/stage2_bridge.py`), both via `sys.path` insert + `sys.modules`-eviction to dodge same-named-module collisions (`models.py` exists in all three stages; Stage 3's own bridge module is named `stage2_bridge.py`, not `ingest.py`, specifically to avoid colliding with Stage 2's own `ingest.py`). A third stage needing this same cross-import has now happened — consolidating into a real package is overdue, not just "worth revisiting."
