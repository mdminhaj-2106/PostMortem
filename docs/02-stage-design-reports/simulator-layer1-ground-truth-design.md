...
- **Uneven effect across segments/regions**: rather than an event applying identically everywhere or entirely to one `affected_segment`, give it a `segment_multiplier` — mostly close to 1.0 (roughly even), occasionally sharply uneven (an outage that's technically global but hits VIP customers harder because they order most often and notice reliability issues first).
